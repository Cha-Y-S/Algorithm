...
### 접근 방식
  ...
  - parameter로 주어진 `board`를 오른쪽으로 90도 회전시킨 행렬을 저장하는 `cBoard`선언

  - `cBoard`에서 `0`을 모두 지움

  - `cBoard`의 `moves[i] - 1`인덱스로 접근

    - `select`와 `basket`의 top이 같으면 `answer`에 2를 더하고 push 하지 않음

      `cBoard[select]` pop_back 수행

    - `select`와 `basekt`의 top이 같지 않으면 push_back

    - `cBoard[select]`가 empty면 아무 작업도 하지 않음