...
| :-------- | :---------- | :--- | :--- | :-------- | :-------- |
| 1 초      | 128 MB      | 1508 | 631  | 509       | 41.997%   |
...
### 문제 접근
  ...
  - 먼저 움직일 수 있는 방향을 설정(오른쪽, 아래)

  - `queue`에 있는 좌표값에 따라 움직이는 스텝 수에 의해 `next_x`와 `next_y` 결정

  - 각각 `nx`, `ny`가 맵의 범위를 벗어나지 않으면 `queue`에 push

  - 새로 `queue`에서 꺼낸 좌표값이 `N-1`, `N-1`이라면 `flag`를 true로 변경하고 함수 종료

  - `main`에서 `flag` 값에 따라 `"HaruHaru"`나 `"Hing"` 출력